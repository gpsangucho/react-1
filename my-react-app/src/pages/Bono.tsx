...
import api from '../api/axiosConfig'; // ← Importa Axios
...
//Inicio cambio
    if (sueldoDiario === '' || diasDisfrute === '') return;

    try {
      const response = await api.post('/bono', {
        sueldoDiario: Number(sueldoDiario),
        diasDisfrute: Number(diasDisfrute),
      });

      setBono(response.data.bono); // El backend devuelve { bono: valor }
      setError(null);
    } catch (err) {
      console.error('Error al calcular bono:', err);
      setError('No se pudo calcular el bono.');
    }
